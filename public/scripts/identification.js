...
async function idSearch(event) {
  ...
  const sparqlQuery = `
    ...
    WHERE {
      ?bird rdf:type dbo:Bird ;
            ...
            dbp:taxon ?taxon.
      FILTER(LANG(?description) = 'en' && (${keywords
        .map((keyword) => `CONTAINS(?name, "${keyword}")`)
        .join(" || ")}))
  ...
  try {
    ...
    for (let i = 0; i < results.results.bindings.length; i++) {
      ...
      // Add data to columns
      col_commonName.innerText =
        identification.commonName !== null ? identification.commonName : "";
      col_scientificName.innerText =
        identification.scientificName !== null
          ? identification.scientificName
          : "";
      col_engDescription.innerText =
        identification.description !== null ? identification.description : "";
      col_link.innerText =
        identification.bird !== null ? identification.bird : "";
